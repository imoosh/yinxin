...
cp "$TOPDIR"/artifact/mbedtls-${MBEDTLS_VER}.tar.gz "$SRCDIR"

docker run -it --net=host -w /opt -v "$SRCDIR":/opt "$BUILD_IMAGE" bash -c "
    export DEBIAN_FRONTEND=noninteractive
    apt update
    apt install -y liblzo2-dev libpam0g-dev liblz4-dev libcap-ng-dev libnl-genl-3-dev linux-libc-dev \
...
cp "$SRCDIR"/openvpn-${OPENVPN_VER}.tar.gz "$TOPDIR"/artifact
